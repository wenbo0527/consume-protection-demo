    ...
    <div v-if="ticket" class="cp-action-body">
      ...
      <div v-if="relatedKnowledge.length" class="cp-know-box">
        ...
        <div v-for="k in relatedKnowledge" :key="k.id" class="cp-know-row">
          <div style="flex: 1; min-width: 0">
            ...
            <div style="font-size: 11px; color: var(--cp-text-tertiary); margin-top: 2px">{{ k.content.slice(0, 60) }}...</div>
          </div>
      ...
      <div class="cp-action-row" style="flex-direction: column; align-items: stretch; gap: 8px">
        ...
        <div v-if="cited.length" class="cp-cite-list">
          <icon-link />
          <a-tag v-for="c in cited" :key="c.id" size="small" closable @close="cited.splice(cited.indexOf(c), 1)">{{ c.title }}</a-tag>
        </div>
      </div>
      ...
      <div class="cp-action-row" style="flex-direction: column; align-items: stretch; gap: 8px">
        ...
        <a-space>
          <a-button @click="wb.removeTask(task.id); Message.success('已转办')">转办</a-button>
          <a-button @click="wb.removeTask(task.id); Message.success('已协办')">协办</a-button>
          <a-button status="warning" @click="wb.removeTask(task.id); Message.success('已升级')">升级</a-button>
        </a-space>
      </div>
    </div>
...
const relatedKnowledge = computed(() => {
  ...
  return knowledge.filter(k =>
    k.relatedCategories?.includes(t.category) ||
    k.relatedReasons?.includes(t.reason)
  ).slice(0, 2)
})
...
.cp-action-meta { display: flex; gap: 6px; }
.cp-action-body { flex: 1; padding: 16px 20px; overflow-y: auto; }
.cp-action-row { display: flex; gap: 12px; margin-bottom: 12px; align-items: flex-start; }
.cp-label { font-size: 12px; color: var(--cp-text-tertiary); min-width: 70px; padding-top: 4px; }
.cp-value { font-size: 13px; color: var(--cp-text); flex: 1; line-height: 1.6; }
.cp-link { font-size: 13px; color: var(--cp-brand); font-family: 'DIN Alternate', monospace; cursor: pointer; }
...
.cp-know-head { display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 500; margin-bottom: 8px; }
.cp-know-row {
  display: flex; align-items: center; gap: 8px;
  padding: 8px 10px; background: #fff;
  border-radius: 4px; margin-bottom: 6px;
}
.cp-cite-list { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; padding: 6px 8px; background: var(--cp-bg-soft); border-radius: 4px; font-size: 12px; color: var(--cp-text-tertiary); }
</style>