<template>
  <div class="cp-page">
      ...
      <a-table :data="filteredTickets" :pagination="{ pageSize: 10 }">
        <template #columns>
          ...
          <a-table-column title="流转时长" :width="100">
            <template #cell="{ record }">
              <span class="mono" :style="{ color: record.duration > 7 ? 'var(--cp-danger)' : record.duration > 5 ? 'var(--cp-warning)' : '' }">
                {{ record.duration }} d
              </span>
            </template>
          </a-table-column>
    ...
    <!-- 流转追踪 Drawer -->
    <a-drawer v-model:visible="timelineVisible" :width="520" :title="`流转追踪 · ${currentTicket?.id}`" placement="right">
      <div v-if="currentTicket">
        <a-alert :type="currentTicket.duration > 7 ? 'error' : currentTicket.duration > 5 ? 'warning' : 'info'" show-icon style="margin-bottom: 16px">
        ...
        <template #columns>
          ...
          <a-table-column title="当前节点">
            <template #cell="{ record }">
              ...
              <span style="font-size: 11px; color: var(--cp-text-tertiary); margin-left: 6px">{{ roleLabel(currentNodeRole(record)) }}处置</span>
              <a-tag v-if="slaProgressOf(record) >= 0.75" size="small" :color="slaProgressOf(record) >= 1 ? 'red' : 'orange'" style="margin-left: 4px">
          ...
          <a-table-column title="副作用回写" data-index="relatedTicketStatus" :width="120">
            <template #cell="{ record }">
              <span v-if="record.relatedTicketStatus" style="color: var(--cp-success); font-size: 12px">{{ record.relatedTicketStatus }}</span>
    ...
    <!-- 业务工作流节点轨迹 Drawer -->
    <a-drawer v-model:visible="bizTimelineVisible" :width="560" :title="bizCurrent ? `${bizCurrent.id} · ${wf.templateByKind(bizCurrent.kind)?.name || ''}` : '节点轨迹'">
      <div v-if="bizCurrent">
        <a-descriptions :column="1" bordered size="small" style="margin-bottom: 16px">
          <a-descriptions-item label="客户">{{ bizCurrent.customerName || '-' }}</a-descriptions-item>
          <a-descriptions-item v-if="bizCurrent.ticketId" label="关联工单">{{ bizCurrent.ticketId }}</a-descriptions-item>
          <a-descriptions-item label="发起">{{ bizCurrent.initiator }} · {{ bizCurrent.createdAt }}</a-descriptions-item>
          <a-descriptions-item label="状态">
            <a-tag :color="statusColor(bizCurrent.status)">{{ statusLabel(bizCurrent.status) }}</a-tag>
          </a-descriptions-item>
          <a-descriptions-item v-if="bizCurrent.relatedTicketStatus" label="副作用回写">{{ bizCurrent.relatedTicketStatus }}</a-descriptions-item>
        </a-descriptions>
        <a-timeline>
          <a-timeline-item v-for="n in (wf.templateByKind(bizCurrent.kind)?.nodes || [])" :key="n.code"
            :color="n.code === bizCurrent.currentNode ? 'var(--cp-brand)' : (execStatusOf(bizCurrent, n.code) === 'approved' ? 'var(--cp-success)' : (execStatusOf(bizCurrent, n.code) === 'rejected' ? 'var(--cp-danger)' : 'gray'))">
            <div style="display: flex; justify-content: space-between">
              <span style="font-weight: 500">{{ n.name }}</span>
              <a-tag size="small">{{ nodeKindShort(n.kind) }}</a-tag>
            </div>
            <div style="font-size: 11px; color: var(--cp-text-tertiary); margin-top: 2px">处置 {{ roleLabel(n.handlerRole) }} · SLA {{ n.slaHours }}h</div>
            <div v-if="execOf(bizCurrent, n.code)?.comment" style="font-size: 12px; margin-top: 4px">
              ...
              · {{ execOf(bizCurrent, n.code)?.operatedAt }}
              · {{ execOf(bizCurrent, n.code)?.comment }}
            </div>
            <div v-if="execOf(bizCurrent, n.code)?.payload && Object.keys(execOf(bizCurrent, n.code)!.payload!).length" style="font-size: 11px; margin-top: 4px; background: var(--cp-bg-soft); padding: 6px 8px; border-radius: 4px">
    ...
    <div class="cp-card" style="padding: 16px 20px; margin-top: 16px">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px">
        ...
        <a-button size="small" type="primary" @click="$router.push('/manage/rectify')">
          进入整改工作台 →
        </a-button>
      </div>
      ...
      <a-table v-else :data="rectify.tasks" :pagination="{ pageSize: 5 }" row-key="id">
        <template #columns>
          ...
          <a-table-column title="沉淀产出">
            <template #cell="{ record }">
              <a-space :size="4">
                <a-tag v-if="record.generatedStandardIds?.length" size="small" color="purple">{{ record.generatedStandardIds.length }} 标准</a-tag>
                <a-tag v-if="record.generatedKbIds?.length" size="small" color="arcoblue">{{ record.generatedKbIds.length }} 知识</a-tag>
                <span v-if="!record.generatedStandardIds?.length && !record.generatedKbIds?.length" style="color: var(--cp-text-tertiary); font-size: 11px">-</span>
              </a-space>
            </template>
          </a-table-column>
        </template>
...
const soonOverdue = computed(() => allTickets.value.filter(t => t.urgency === 'urgent' && t.status !== 'closed').length)
...
const enrichedTickets = computed(() => allTickets.value.map((t, i) => ({
  ...t,
  duration: t.status === 'closed' ? Math.floor(Math.random() * 5) + 1 : Math.floor(Math.random() * 6) + 2,
  nextStep: t.status === 'todo' ? '分配坐席' : t.status === 'processing' ? '客户回复/转办' : t.status === 'closing' ? '坐席确认关单' : '已归档'
})))
...
function nodeKindColor(k: string) {
  return { apply: 'arcoblue', approve: 'orange', execute: 'green', notify: 'gray', auto: 'gray', archive: 'gray' }[k] || 'gray'
}
function nodeKindShort(k: string) {
  return { apply: '申请', approve: '审批', execute: '执行', notify: '通知', auto: '自动', archive: '归档' }[k] || k
}
const ROLE_LABELS: Record<string, string> = { agent: '坐席', business: '支撑岗', review: '审查', manage: '管理层', system: '系统' }
...
.cp-status-count { font-size: 24px; font-weight: 600; margin-bottom: 4px; }
.cp-status-label { font-size: 12px; color: var(--cp-text-secondary); }
.cp-status-percent { font-size: 11px; color: var(--cp-text-tertiary); margin-top: 2px; }
.cp-status-bar {
  margin-top: 8px;
  height: 4px;
