<template>
  <a-layout class="cp-layout">
    <!-- 浅色侧栏(Arco 企业最佳实践) -->
    <a-layout-sider :width="collapsed ? 64 : 220" :collapsed="collapsed" :collapsible="true" class="cp-sider" theme="light">
      ...
      >
        <a-sub-menu v-for="group in menus" :key="group.key">
          <template #title>
            <span><component :is="group.icon" /> <span v-show="!collapsed">{{ group.title }}</span></span>
          </template>
          ...
          <a-space :size="4">
            ...
            <a-dropdown trigger="hover">
              <div class="cp-user-chip">
                <a-avatar :size="28" style="background: var(--cp-brand); font-size: 12px">{{ currentRoleInfo?.username?.charAt(0) }}</a-avatar>
              ...
              <template #content>
                <a-doption @click="showSwitch = true">
                  <icon-swap /> 切换角色
                </a-doption>
                <a-doption>
                  <icon-settings /> 个人设置
...
interface MenuItem { name: string; path: string; badge?: number }
  ...
  switch (userStore.currentRole) {
    case 'agent':
      return [
        ...
        { key: 'g2', title: '客户', icon: 'icon-user', items: [
          { name: '客户画像查询', path: '/agent/customer-search' }
        ]},
        { key: 'g3', title: '知识', icon: 'icon-book', items: [
    ...
    case 'business':
      return [
        ...
        { key: 'g2', title: '停催停扣', icon: 'icon-pause-circle', items: [
          { name: '停催申请', path: '/business/stop-coll' }
        ]},
        { key: 'g3', title: '协商还款', icon: 'icon-handshake', items: [
          { name: '协商方案管理', path: '/business/negotiate' }
        ]},
        { key: 'g4', title: '征信与清退', icon: 'icon-credit-card', items: [
          { name: '征信异议处理', path: '/business/credit' }
        ]},
        { key: 'g5', title: '转诉与调解', icon: 'icon-send', items: [
          { name: '转诉调解工作区', path: '/business/transfer' }
        ]}
      ]
    case 'review':
      return [
        { key: 'g1', title: '审查立项', icon: 'icon-file', items: [
          { name: '待审查立项', path: '/review/pending' },
          { name: '创建立项', path: '/review/create' }
        ]},
        { key: 'g2', title: '审查标准', icon: 'icon-storage', items: [
          { name: '标准列表/维护', path: '/review/standards' }
        ]},
    ...
    case 'manage':
      return [
        { key: 'g1', title: '驾驶舱', icon: 'icon-dashboard', items: [
          { name: '消保看板', path: '/manage/dashboard' },
          { name: '预警中心', path: '/manage/alert', badge: 5 },
          { name: '不满意评价', path: '/manage/alert' }
        ]},
        { key: 'g2', title: '规则配置', icon: 'icon-settings', items: [
          { name: '分单/预警/标签/名单', path: '/manage/rules' }
        ]},
        { key: 'g3', title: '名单管理', icon: 'icon-storage', items: [
          { name: '黑名单/投诉库/代理库', path: '/manage/lists' }
        ]},
        { key: 'g4', title: '知识管理', icon: 'icon-book', items: [
          { name: '知识条目管理', path: '/manage/knowledge' }
        ]},
        { key: 'g5', title: '流程管理', icon: 'icon-swap', items: [
          { name: '工单流程配置', path: '/manage/workflow-config' },
          { name: '工单流转监控', path: '/manage/workflow-monitor' }
        ]},
        { key: 'g5', title: '运营管理', icon: 'icon-tool', items: [
          { name: '排班/绩效/请假', path: '/manage/ops' },
          { name: '质检管理', path: '/manage/quality' },
          { name: '贷中清退', path: '/manage/exit' },
          { name: '票据合同', path: '/manage/billing' }
        ]},
        { key: 'g7', title: '溯源整改', icon: 'icon-reload', items: [
          { name: '整改工作台', path: '/manage/rectify' }
        ]}
      ]
    case 'consumer':
      return [
        { key: 'g1', title: '我的投诉', icon: 'icon-form', items: [
          { name: '投诉进度查询', path: '/consumer/complaints' }
        ]},
        { key: 'g2', title: '评价反馈', icon: 'icon-star', items: [
          { name: '满意度评价', path: '/consumer/feedback' }
        ]}
      ]
    default:
      return []
...
.cp-sider :deep(.arco-menu-light) { background: transparent; }
...
.cp-sider :deep(.arco-menu-sub-menu-title) { color: var(--cp-text-secondary); font-weight: 500; }
...
.cp-logo.is-collapsed { justify-content: center; padding: 14px 0; }
.cp-logo-icon {
...
.cp-logo-text { flex: 1; min-width: 0; line-height: 1.3; }
.cp-logo-title { color: var(--cp-text); font-size: 14px; font-weight: 600; }
.cp-logo-sub { color: var(--cp-text-tertiary); font-size: 11px; margin-top: 2px; }
...
.cp-header-left :deep(.arco-breadcrumb-item) { font-size: 13px; }
.cp-header-right { display: flex; align-items: center; }
...
.cp-user-chip:hover { background: var(--cp-bg-hover); }
.cp-user-info { display: flex; flex-direction: column; line-height: 1.3; }
.cp-user-name { font-size: 13px; font-weight: 500; color: var(--cp-text); }
.cp-user-role { font-size: 11px; color: var(--cp-text-tertiary); }
...
.cp-role-card {
  border: 1px solid var(--cp-border-light); border-radius: 6px;
...
.cp-role-name { font-size: 14px; font-weight: 600; margin-top: 10px; color: var(--cp-text); }
.cp-role-work { font-size: 12px; color: var(--cp-brand); margin-top: 2px; }
.cp-role-desc { font-size: 11px; color: var(--cp-text-tertiary); margin-top: 6px; line-height: 1.5; }
.cp-role-card :deep(.arco-tag) { position: absolute; top: 8px; right: 8px; }
