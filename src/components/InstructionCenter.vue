  ...
  <div v-if="myList.length" class="cp-instruction-bar">
    <div class="cp-instruction-bar-inner">
      <div class="cp-instruction-bar-title">
        ...
        <span style="color: var(--cp-text-tertiary); font-size: 12px"> 当前待办 {{ pendingCount }} 条 </span>
      </div>
      ...
      <div class="cp-instruction-list">
        ...
        >
          ...
          <div
            v-if="i.alertId || i.ticketId"
            style="font-size: 12px; color: var(--cp-text-tertiary); margin-bottom: 6px"
          >
            <span v-if="i.alertId"
              >预警 <a-link>{{ i.alertId }}</a-link></span
            >
            <span v-if="i.ticketId" style="margin-left: 8px"
              >工单 <a-link>{{ i.ticketId }}</a-link></span
            >
          </div>
          <div style="display: flex; gap: 8px">
            <a-button v-if="i.status === 'pending'" type="primary" size="mini" @click="ack(i)">确认接收</a-button>
            <a-button
        ...
        <template #item="{ item: ins, index }">
          <a-list-item :key="index">
            <div style="width: 100%">
              ...
              <div v-if="ins.ackAt" style="font-size: 12px; color: var(--cp-text-tertiary); margin-top: 2px">
                ack {{ ins.ackAt }} · {{ ins.ackNote || '' }}
              </div>
              <div v-if="ins.doneAt" style="font-size: 12px; color: var(--cp-success); margin-top: 2px">
                done {{ ins.doneAt }} · {{ ins.doneNote || '' }}
              </div>
              <div style="display: flex; gap: 6px; margin-top: 8px">
                <a-button v-if="ins.status === 'pending'" type="primary" size="mini" @click="ack(ins)">确认</a-button>
                <a-button
                  v-if="ins.status === 'pending' || ins.status === 'ack'"
                  type="primary"
                  size="mini"
                  status="success"
                  @click="done(ins)"
...
const pendingCount = computed(() => myList.value.filter((i) => i.status === 'pending').length)
...
.cp-instruction-normal {
  border-left-color: #1494e8;
  background: rgba(20, 148, 232, 0.03);
}
.cp-instruction-low {
