...
const DEFAULT_TEMPLATES: WorkflowTemplate[] = [
  {
    ...
    enabled: true,
    nodes: [
      { code: 'apply', name: '坐席发起申请', kind: 'apply', handlerRole: 'agent', slaHours: 1, autoNext: true, fields: [
      ...
      { code: 'approve', name: '支撑岗审批', kind: 'approve', handlerRole: 'business', slaHours: 4, autoNext: false, sideEffect: 'stop_collection_active' },
      { code: 'effective', name: '催收/扣款系统生效', kind: 'auto', handlerRole: 'system', slaHours: 0, autoNext: true },
      { code: 'notify', name: '通知原坐席', kind: 'notify', handlerRole: 'system', slaHours: 0, autoNext: true, sideEffect: 'notify_seat' },
  ...
  },
  {
    ...
    enabled: true,
    nodes: [
      { code: 'apply', name: '坐席/支撑岗发起', kind: 'apply', handlerRole: 'agent', slaHours: 1, autoNext: true, fields: [
      ...
      { code: 'approve', name: '支撑岗审批', kind: 'approve', handlerRole: 'business', slaHours: 4, autoNext: false, sideEffect: 'negotiate_active' },
      { code: 'effective', name: '自动触发停催停扣', kind: 'auto', handlerRole: 'system', slaHours: 0, autoNext: true, sideEffect: 'stop_collection_active' },
      { code: 'archive', name: '归档', kind: 'archive', handlerRole: 'system', slaHours: 0, autoNext: false }
    ]
  },
  {
    ...
    enabled: true,
    nodes: [
      { code: 'apply', name: '坐席/支撑岗提交', kind: 'apply', handlerRole: 'agent', slaHours: 2, autoNext: true, fields: [
        { key: 'platform', label: '调解平台', type: 'select', options: ['金融调解委员会', '仲裁机构', '法院诉前调解'] },
      ...
      { code: 'close', name: '结案关联工单', kind: 'archive', handlerRole: 'business', slaHours: 0, autoNext: false, sideEffect: 'notify_seat' }
    ]
  },
  {
    ...
    enabled: true,
    nodes: [
      { code: 'apply', name: '坐席登记异议', kind: 'apply', handlerRole: 'agent', slaHours: 1, autoNext: true, fields: [
        { key: 'dispute', label: '异议内容', type: 'text' }
      ]},
  ...
  },
  {
    ...
    enabled: true,
    nodes: [
      { code: 'archive', name: '审查人员归档', kind: 'archive', handlerRole: 'review', slaHours: 1, autoNext: true, sideEffect: 'archive_to_kb' },
      { code: 'kb_review', name: '知识管理员审核', kind: 'approve', handlerRole: 'manage', slaHours: 8, autoNext: true },
      { code: 'notify', name: '通知坐席(知识更新)', kind: 'notify', handlerRole: 'system', slaHours: 0, autoNext: true, sideEffect: 'notify_seat' }
    ]
  },
  {
    ...
    enabled: true,
    nodes: [
      { code: 'confirm', name: '管理层确认预警', kind: 'approve', handlerRole: 'manage', slaHours: 2, autoNext: true, fields: [
      ...
      { code: 'verify', name: '预警标记已验证', kind: 'archive', handlerRole: 'system', slaHours: 0, autoNext: false, sideEffect: 'mark_alert_verified' }
    ]
  }
]
  ...
  return [
    {
      id: 'WF-20260715-0001',
      kind: 'stop_collection',
      customerId: 'C003', customerName: '周志远', ticketId: 'GD-20260714-0008',
      initiator: '张敏', initiatorRole: 'agent',
      createdAt: '2026-07-15 14:20', currentNodeStartedAt: '2026-07-15 14:20', expireAt: '2026-07-15 18:20', currentNode: 'approve', status: 'running',
      executions: [
        { nodeCode: 'apply', status: 'done', operator: '张敏', operatedAt: '2026-07-15 14:20', comment: '客户扬言投诉,协商前停催', payload: { reason: '客户扬言投诉', period: '15 天', syncStopDeduct: '是' } }
      ]
    },
    {
      id: 'WF-20260712-0003',
      kind: 'negotiate',
      customerId: 'C001', customerName: '刘建国', ticketId: 'GD-20260712-0021',
      initiator: '李伟', initiatorRole: 'business',
      createdAt: '2026-07-12 09:30', currentNodeStartedAt: '2026-07-12 13:31', expireAt: '2026-07-12 17:30', currentNode: 'effective', status: 'approved',
      executions: [
        { nodeCode: 'apply', status: 'done', operator: '李伟', operatedAt: '2026-07-12 09:30', payload: { loanId: 'LN-2024-001', plan: '分 6 期,前低后高' } },
        { nodeCode: 'trial_calc', status: 'done', operator: '系统', operatedAt: '2026-07-12 09:31' },
        { nodeCode: 'approve', status: 'approved', operator: '陈强', operatedAt: '2026-07-12 10:15', comment: '同意,按方案执行' }
      ]
    },
    {
      id: 'WF-20260710-0002',
      kind: 'transfer_mediate',
      customerId: 'C005', customerName: '王秀英',
      initiator: '张敏', initiatorRole: 'agent',
      createdAt: '2026-07-10 11:00', currentNodeStartedAt: '2026-07-11 11:00', expireAt: '2026-07-12 11:00', currentNode: 'sync', status: 'running',
      executions: [
        { nodeCode: 'apply', status: 'done', operator: '张敏', operatedAt: '2026-07-10 11:00', payload: { platform: '金融调解委员会', caseNo: 'JD-202607-088' } },
...
export const useWorkflowStore = defineStore('workflow', {
  ...
  getters: {
    ...
    /** 支撑岗待办 */
    businessTodos: (s) => s.instances.filter(i => {
      if (i.status !== 'running') return false
      const tpl = s.templates.find(t => t.kind === i.kind)
      if (!tpl) return false
      const node = tpl.nodes.find(n => n.code === i.currentNode)
    ...
    /** 管理层待办(指令接收 / 知识审核) */
    manageTodos: (s) => s.instances.filter(i => {
      if (i.status !== 'running') return false
      const tpl = s.templates.find(t => t.kind === i.kind)
      if (!tpl) return false
      const node = tpl.nodes.find(n => n.code === i.currentNode)
  ...
  actions: {
    ...
    start(input: {
      ...
      const expireAt = totalSlaHours > 0
        ...
        executions: [
          {
            ...
            status: firstNode.kind === 'auto' || firstNode.kind === 'notify' || firstNode.kind === 'archive' ? 'done' : 'running',
    ...
    _runSideEffect(effect: NonNullable<WorkflowNode['sideEffect']>, inst: WorkflowInstance, tpl: WorkflowTemplate) {
      ...
      switch (effect) {
        ...
        case 'notify_seat':
          window.dispatchEvent(new CustomEvent(EVT.WORKFLOW_NOTIFY_SEAT, {
        ...
        case 'mark_alert_verified':
          ...
          window.dispatchEvent(new CustomEvent(EVT.WORKFLOW_ALERT_VERIFIED, {
    ...
    tickOverdue() {
      ...
      this.instances.forEach(inst => {
        ...
        if (now > exp) {
          ...
          window.dispatchEvent(new CustomEvent(EVT.WORKFLOW_OVERDUE, {
